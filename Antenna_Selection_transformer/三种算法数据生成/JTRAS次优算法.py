...
#数据预处理
dataset = pd.read_csv(open(r'D:\qy-transformer\AntennaTransformer-gai\data\全信道矩阵(p=10)-无标签.csv')).iloc[:, 1:]
...
Capacity_Mean = np.mean(subCapacity_List)

End = time.perf_counter()

print("基于信道容量 信噪比 ", p)
print("JTRAS算法_8822(40000个样本)")
# print("40000个样本的测试时间 %.1f %s" % (computation_time(sum(testTime))[0], computation_time(sum(testTime))[1]))
print('JTRAS子信道容量均值', Capacity_Mean)
# print('预测损失均值', Loss_Mean)
# print('预测损失方差', Loss_Variance)

subCapacityData.to_csv(r'D:\qy-transformer\AntennaTransformer-gai\data\子信道容量(p=10)-无标签.csv')
capacityLabelData.to_csv(r'D:\qy-transformer\AntennaTransformer-gai\data\容量标签(p=10)-无标签.csv')
subGainData.to_csv(r'D:\qy-transformer\AntennaTransformer-gai\data\子信道增益(p=10)-无标签.csv')
gainLabelData.to_csv(r'D:\qy-transformer\AntennaTransformer-gai\data\增益标签(p=10)-无标签.csv')

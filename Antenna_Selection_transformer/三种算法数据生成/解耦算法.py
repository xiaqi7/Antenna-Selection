...
dataset = pd.read_csv(open(r"D:\qy-transformer\AntennaTransformer-gai\data\全信道矩阵(p=50).csv")).iloc[:, 1:]
# 变量类型转换
dataset = np.asarray(dataset, np.float32)
dataset = dataset.reshape(dataset.shape[0], 8, 8, 1)
# .iloc[:, 1]  只取第2列(因为一开始会有一个索引))
label = pd.read_csv(open(r"D:\qy-transformer\AntennaTransformer-gai\data\容量信道标签计数(p=50).csv")).iloc[:, 1]
# 变量类型转换

...
class Model(nn.Module):
    def __init__(self, inputs_size, outputs_size):   # 64   786，表示规定输入输出数量  64个输入坑位
        super(Model, self).__init__()
        self.dim_up = nn.Linear(inputs_size, 256)  # 升维层
        self.positional_encoding = PositionalEncoding(256, 0.2)  # 位置编码层
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=4)  # transformer encoder 单层   d_model一般特征维度
    ...
    def forward(self, inputs):  # torch.Size([B, 16, 2])    (512,1,64)    这个输入是数据默认（B，序列长度，特征维度），传入模型的是特征维度，即64个坑位，最后一维
